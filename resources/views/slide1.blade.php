...
<body class="d-flex flex-column min-vh-100">
    ...
    <header class="top-header d-flex justify-content-between align-items-center px-3 py-2">
        <div class="logo d-flex align-items-center">
            <img src="{{ asset('images/llogo.png') }}" alt="Logo" height="70" class="me-2">
        ...
        <div class="language-switch">
            <select>
                <option value="{{ route('slide1', ['lang' => 'id']) }}" {{ app()->getLocale() == 'id' ? 'selected' : '' }}>Bahasa Indonesia</option>
                <option value="{{ route('slide1', ['lang' => 'en']) }}" {{ app()->getLocale() == 'en' ? 'selected' : '' }}>English</option>
            </select>
        </div>
    </header>
        ...
        <div class="search-bar mt-4">
            <form class="input-group" action="{{ route('search') }}" method="GET">
                <select name="category" class="form-select form-select-lg" style="max-width: 200px;">
                    ...
                    <option value="peraturan">{{ __('Peraturan') }}</option>
                    <option value="monografi">{{ __('Monografi') }}</option>
                    <option value="artikel">{{ __('Artikel') }}</option>
                    <option value="putusan">{{ __('Putusan') }}</option>
                </select>
